...
test("builds the Carbon UI contract", async () => {
  ...
  const plots = await readFile(
    new URL("../src/AnalysisPlots.tsx", import.meta.url),
    "utf8",
  );
  ...
  const carbon = await readFile(
    new URL("../src/carbon.scss", import.meta.url),
    "utf8",
  );
  ...
  assert.match(plots, /aria-describedby=\{descriptionId\}/);
  assert.match(plots, /scientific-visually-hidden/);
  for (const summary of [
    ...
    assert.match(plots, new RegExp(`function ${summary}\\(`));
  }
});
...
test("waits for explicit user action before solving modes", async () => {
  ...
  assert.doesNotMatch(
    app,
    /runSolverWorker<SolverResult>\(\{ kind: "solve", config: initialConfig \}\)/,
  );
});
...
test("reveals results and reports solver state after an explicit solve", async () => {
  ...
  for (const label of ["Not solved", "Solving", "Solved", "Stale"])
...
test("keeps the scientific result ahead of introductory chrome", async () => {
  ...
  assert.doesNotMatch(
    app,
    /view-heading"><div className="view-title">\{icon\}<h1 id=\{id\}>\{title\}<\/h1><\/div><p>/,
  );
});
...
test("keeps React as the sole owner of the application UI root", async () => {
  ...
  const plotConfig = await readFile(
    new URL("../src/plotConfig.ts", import.meta.url),
    "utf8",
  );
  ...
  const bodyWithoutRuntime = body
    .replace(/<div\s+id=["']root["']\s*>\s*<\/div>/i, "")
  ...
  assert.doesNotMatch(
    app,
    /document\.querySelector(?:<[^>]+>)?\(\s*["']#mode-solver-form["']\s*\)[\s\S]{0,80}requestSubmit\s*\(/,
  );
